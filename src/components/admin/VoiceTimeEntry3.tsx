...
type Gate = { kind: "unsupported" | "denied" | "error" | "ready"; msg: string } | null;

export function VoiceTimeEntry3({
  ...
  async function captureFlow(initial: Slots, skipCase: boolean): Promise<void> {
    ...
    setOpen(true); setSaved(false); setCandidates([]); setHeard(""); setInterim(""); setGate(null);
        ...
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-end sm:items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) cancel(); }}>
          <div className="bg-[var(--c-surface)] rounded-lg w-full max-w-md p-6 shadow-2xl max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-[family-name:var(--font-display)] text-lg flex items-center gap-2"><Mic size={18} className="text-[var(--c-accent)]" /> Voice entry <span className="text-[10px] font-semibold text-[var(--c-accent)] border border-[var(--c-accent)] rounded px-1 py-0.5">3.0</span></h3>
              <div className="flex items-center gap-1.5">
                <button onClick={() => setShowSettings((v) => !v)} aria-label="Voice settings" title="Choose the voice and speed"
                ...
                <button onClick={toggleMute} aria-label={muted ? "Unmute" : "Mute"} title={muted ? "Voice is muted" : "Mute the voice"}
                  ...
                  {muted ? <VolumeX size={14} /> : <Volume2 size={14} />}{muted ? "Muted" : "Mute"}
                </button>
            ...
            {gate && (
              <div className={`mb-3 rounded-md border p-3 text-xs ${gate.kind === "ready" ? "border-[var(--c-success)]" : "border-[var(--c-accent)]"} bg-[var(--c-surface2)]`}>
                <p className="flex items-start gap-1.5 text-[var(--c-ink)] leading-relaxed">
                  {gate.kind === "ready" ? <Check size={14} className="mt-0.5 shrink-0 text-[var(--c-success)]" /> : <AlertTriangle size={14} className="mt-0.5 shrink-0 text-[var(--c-accent)]" />}
                  <span>{gate.msg}</span>
                </p>
                {gate.kind !== "unsupported" && (
                  <button onClick={run} className="mt-2 rounded-md bg-[var(--c-accent)] px-3 py-1.5 text-xs font-semibold text-[var(--c-on-accent)] hover:opacity-90">
                    {gate.kind === "ready" ? "Start" : "Try again"}
                  </button>
                )}
                <p className="mt-2 text-[10px] text-[var(--c-ink-muted)]">You can also just type the entry below.</p>
              </div>
            )}
                ...
                <div className="space-y-1.5">
                  {candidates.map((c) => (
                    <div key={c.displayNumber} className="flex items-stretch rounded-md border border-[var(--c-border)] bg-[var(--c-bg)] overflow-hidden hover:border-[var(--c-accent)]">
                      <button onClick={() => setOpenDesc((o) => (o === c.displayNumber ? null : c.displayNumber))} className="flex-1 text-left px-3 py-2 text-sm hover:bg-[var(--c-surface2)]">
                        <span className="flex items-center gap-1.5">{c.displayNumber}<Info size={12} className="text-[var(--c-ink-muted)]" /></span>
                        {openDesc === c.displayNumber && <span className="mt-1 block text-xs text-[var(--c-ink-muted)] leading-relaxed">{c.description || "No description on file for this case."}</span>}
                      </button>
                      <button onClick={() => tapCandidate(c.displayNumber)} aria-label={`Select ${c.displayNumber}`} title="Use this case" className="w-1/5 min-w-[46px] border-l border-[var(--c-border)] flex items-center justify-center text-[var(--c-ink-muted)] hover:bg-[var(--c-success)] hover:text-white">
                        <Check size={18} />
                      </button>
                    </div>
                  ))}
            ...
            {verifying && candidates.length === 0 && (
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button onClick={() => press(true)} className="flex items-center justify-center gap-1.5 rounded-md bg-[var(--c-success)] px-4 py-3 text-base font-semibold text-white hover:opacity-90"><Check size={18} /> {labels.yes}</button>
                <button onClick={() => press(false)} className="flex items-center justify-center gap-1.5 rounded-md bg-[var(--c-error)] px-4 py-3 text-base font-semibold text-white hover:opacity-90"><X size={18} /> {labels.no}</button>
              </div>
            )}
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input list="vte3-matters" value={slots.matter ?? ""} onChange={(e) => upd({ matter: e.target.value })} className={fieldClass} placeholder="Choose a case" />
                <datalist id="vte3-matters">{matters.map((m) => <option key={m.displayNumber} value={m.displayNumber}>{m.description}</option>)}</datalist>
                {descOf(slots.matter) && <p className="mt-1 text-xs text-[var(--c-ink-muted)] leading-relaxed">{descOf(slots.matter)}</p>}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelClass}>Hourly rate</label><input type="number" step="1" value={slots.rate ?? ""} onChange={(e) => upd({ rate: e.target.value === "" ? undefined : parseFloat(e.target.value) })} className={fieldClass} /></div>
                <div><label className={labelClass}>Date</label><input type="date" value={slots.date ?? todayISO()} onChange={(e) => upd({ date: e.target.value })} className={fieldClass} /></div>
                <div><label className={labelClass}>Hours</label><input type="number" step="0.1" value={slots.hours ?? ""} onChange={(e) => upd({ hours: e.target.value === "" ? undefined : parseFloat(e.target.value) })} className={fieldClass} /></div>
                <div><label className={labelClass}>Category</label><select value={slots.category ?? ""} onChange={(e) => upd({ category: e.target.value })} className={fieldClass}><option value="">—</option>{categories.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
              </div>
              <div><label className={labelClass}>Activity note</label><textarea rows={3} value={slots.notes ?? ""} onChange={(e) => upd({ notes: e.target.value })} className={fieldClass} placeholder="What was done" /></div>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!slots.nonBillable} onChange={(e) => upd({ nonBillable: e.target.checked })} className="accent-[var(--c-accent)]" /> Non-billable</label>
            </div>

            {saved && <p className="mt-4 text-sm text-[var(--c-success)] flex items-center gap-1"><Check size={15} /> Added to your board.</p>}

            <div className="mt-5 flex justify-end gap-2">
              {!saved && !verifying && <button onClick={saveReview} className="flex items-center justify-center gap-1.5 rounded-md bg-[var(--c-success)] px-4 py-2 text-sm font-semibold text-white hover:opacity-90"><Check size={16} /> Save entry</button>}
            ...
            <div className="mt-3 border-t border-[var(--c-border)] pt-2">
              <button onClick={() => setShowDiag((v) => !v)} className="text-[11px] text-[var(--c-ink-muted)] underline">{showDiag ? "Hide" : "Show"} technical details</button>
              {showDiag && (

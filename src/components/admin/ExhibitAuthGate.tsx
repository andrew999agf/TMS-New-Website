...
import { requestExhibitCode, exhibitCodeLogin } from "@/app/exhibits/r/[token]/auth-actions";

/**
 * The recipient gate: prove you own the invited email with a one-time code before
 * any exhibit loads. A forwarded link alone won't get anyone in.
 */
export function ExhibitAuthGate({ token, email }: { token: string; email: string }) {
  ...
  function sendCode() {
    setError(null);
    start(async () => {
      const r = await requestExhibitCode(token);
  ...
  function verify() {
    setError(null);
    start(async () => {
      const r = await exhibitCodeLogin(token, code);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6">
      <div className="rounded-xl border border-[var(--c-border)] bg-[var(--c-surface)] p-6 shadow-sm">
        <div className="mb-3 inline-flex items-center gap-2 text-[var(--c-accent)]"><ShieldCheck size={18} /> <span className="text-xs font-semibold uppercase tracking-[0.14em]">Secure exhibits</span></div>
        <h1 className="font-[family-name:var(--font-display)] text-xl text-[var(--c-ink)]">Verify it&apos;s you</h1>
        <p className="mt-2 text-sm text-[var(--c-ink-muted)]">These exhibits were shared privately with <strong className="text-[var(--c-ink)]">{email}</strong>. We&apos;ll email a one-time code to confirm before opening them.</p>
        ...
        <div className="mt-5 space-y-3">
          {!sent ? (
            ...
            <>
              ...
              <button onClick={verify} disabled={pending || code.length !== 6} className={primary}>{pending ? <Loader2 size={15} className="animate-spin" /> : <ShieldCheck size={15} />} Verify &amp; open</button>

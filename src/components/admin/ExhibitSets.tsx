...
export function ExhibitSets({ sets, matters, discoveryMatters = [], hub = {} }: { sets: SetRow[]; matters: MatterOption[]; discoveryMatters?: string[]; hub?: Record<string, { name: string; causeNumber: string; court: string }> }) {
  ...
  function submit() {
    ...
    start(async () => {
      const r = await createExhibitSet(f, alsoDiscovery && !discoveryExists);
          ...
          <div className="mt-3">
            {discoveryExists ? (
              <p className="inline-flex items-center gap-1.5 rounded-md bg-emerald-500/10 px-2.5 py-1.5 text-xs font-medium text-emerald-700 dark:text-emerald-300">
                A discovery case for matter {f.matter.trim()} already exists — this set will link to it.
              </p>
            ) : (
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={alsoDiscovery} onChange={(e) => setAlsoDiscovery(e.target.checked)} className="accent-[var(--c-accent)]" />
                Also set up the matching <strong>discovery case</strong> in the Discovery Reviewer
              </label>
            )}
          </div>

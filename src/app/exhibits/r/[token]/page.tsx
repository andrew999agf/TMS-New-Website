...
export default async function RecipientIndex({ params }: { params: Promise<{ token: string }> }) {
  ...
  const groups = ["plaintiff", "defendant", "joint"].map((s) => ({ side: s, items: ordered.filter((d) => d.side === s) })).filter((g) => g.items.length);
          ...
          {groups.map((g) => (
            <section key={g.side}>
              ...
              <ul className="divide-y divide-[var(--c-border)] overflow-hidden rounded-lg border border-[var(--c-border)]">
                {g.items.map((d) => (
                  <li key={d.id}>
                    <Link href={`/exhibits/r/${token}/e/${d.id}`} className="flex items-start gap-3 bg-[var(--c-surface)] px-4 py-3 hover:bg-[var(--c-surface2)]">

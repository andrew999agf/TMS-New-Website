...
import { getOcSet, orderPublicDocs } from "@/lib/exhibit-review/public";
...
function Unavailable() {
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-3 text-sm text-[var(--c-ink-muted)]">The exhibit link is turned off or no longer exists. Ask whoever sent it for a current link.</p>
    </main>
  );
}

export default async function OpposingCounselIndex({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const set = await getOcSet(token);
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-10">
      <header className="border-b border-[var(--c-border)] pb-5">
        ...
        <p className="mt-1 text-sm text-[var(--c-ink-muted)]">{[set.causeNumber, set.court].filter(Boolean).join("  ·  ")}</p>
      </header>

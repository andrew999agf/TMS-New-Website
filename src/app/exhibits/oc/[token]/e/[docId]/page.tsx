...
function Unavailable() {
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-3 text-sm text-[var(--c-ink-muted)]">The exhibit link is turned off or no longer exists.</p>
    </main>
  );
}

export default async function OpposingCounselView({ params }: { params: Promise<{ token: string; docId: string }> }) {
  const { token, docId } = await params;
  const set = await getOcSet(token);
  ...
  return (
    <main className="flex h-screen flex-col bg-[var(--c-bg)]">
      <header className="flex flex-wrap items-center gap-2 border-b border-[var(--c-border)] bg-[var(--c-surface)] px-4 py-2.5">
        <Link href={`/exhibits/oc/${token}`} className="inline-flex items-center gap-1.5 rounded-md border border-[var(--c-border)] px-2.5 py-1.5 text-xs text-[var(--c-ink-muted)] hover:text-[var(--c-accent)]" title="All exhibits"><List size={14} /> All exhibits</Link>
        <div className="min-w-0 flex-1 truncate text-sm font-semibold text-[var(--c-ink)]"><span className="text-[var(--c-accent)]">{d.label || (d.number ?? "")}</span>{d.label || d.number != null ? " — " : ""}{d.title || "Exhibit"}</div>
        <div className="flex items-center gap-1">
          {prev ? <Link href={`/exhibits/oc/${token}/e/${prev.id}`} className="rounded-md border border-[var(--c-border)] p-1.5 text-[var(--c-ink-muted)] hover:text-[var(--c-accent)]" title="Previous"><ChevronLeft size={16} /></Link> : <span className="rounded-md border border-[var(--c-border)] p-1.5 opacity-30"><ChevronLeft size={16} /></span>}
      ...
      {d.isVideo ? (
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center bg-black">
          <video src={`/exhibits/oc/${token}/file/${d.id}`} controls playsInline preload="metadata" className="max-h-full max-w-full" />
        </div>
      ) : (
        <>

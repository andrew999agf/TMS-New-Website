...
import { VoiceTimeEntry3 } from "@/components/admin/VoiceTimeEntry3";
...
/**
 * Time Tracker 3.0 — identical tracker (same database, same CSV), with the 3.0
 * voice engine built on a real microphone-permission state machine. Lives
 * alongside the original and 2.0 so the proven versions remain available.
 */
export default async function TimeTracker3Page() {
  ...
  return (
    <>
      <AdminHeader
        title="Time Tracker 3.0"
        description="Same tracker and CSV, with the rebuilt voice engine (robust microphone setup). The original Time Tracker stays available as a fallback."
      />
      <div className="p-8">
        <TimeTracker
          entries={entries}
          ...
          owners={owners}
          VoiceComponent={VoiceTimeEntry3}
        />
      </div>
    </>
  );

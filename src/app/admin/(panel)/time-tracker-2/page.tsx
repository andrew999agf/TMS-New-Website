...
import { VoiceTimeEntry2 } from "@/components/admin/VoiceTimeEntry2";
...
/**
 * Time Tracker 2.0 — identical tracker (same database, same CSV import/export),
 * but with the experimental 2.0 voice engine. Lives alongside the original so
 * the firm always has the proven version to fall back on.
 */
export default async function TimeTracker2Page() {
  ...
  return (
    <>
      <AdminHeader
        ...
        description="Same tracker and CSV, with the new voice engine we're trying out. The original Time Tracker stays available as a fallback."
      />
      <div className="p-8">
        <TimeTracker
          entries={entries}
          ...
          owners={owners}
          VoiceComponent={VoiceTimeEntry2}
        />
      </div>
    </>
  );
